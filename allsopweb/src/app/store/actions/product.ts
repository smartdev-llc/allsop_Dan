...
export class ProductActionService {
  ...
  static addOrUpdate = createAction(ACTION_STRING.addOrUpdateProducts, props<IActionData>());
  addOrUpdate(list: any, description: string = '') {
    if (!list) {
      return;
    }
    const actionData = getActionData(list, description);
    return this.store.dispatch(ProductActionService.addOrUpdate(actionData));
  }
}
