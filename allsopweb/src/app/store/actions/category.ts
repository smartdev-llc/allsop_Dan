...
export class CategoryActionService {
  ...
  static addOrUpdate = createAction(ACTION_STRING.addOrUpdateCategories, props<IActionData>());
  addOrUpdate(list: any, description: string = '') {
    if (!list) {
      return;
    }
    const actionData = getActionData(list, description);
    return this.store.dispatch(CategoryActionService.addOrUpdate(actionData));
  }
}
